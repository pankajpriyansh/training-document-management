...
public class TraineeController {
	...
	/*
	 * @RequestMapping(value = "/requestController") public void
	 * getMessageDataFromMailFormAndSendMessage(
	 * 
	 * @ModelAttribute("mail") Mail mail, HttpServletResponse response,
	 * HttpSession session) throws IOException {
	 * 
	 * ClassLoader classLoader = Thread.currentThread()
	 * .getContextClassLoader(); InputStream input = classLoader
	 * .getResourceAsStream("credentialsForMail.properties"); Properties prop =
	 * new Properties(); prop.load(input); String email =
	 * prop.getProperty("email"); String password =
	 * prop.getProperty("password"); System.out.println(email +
	 * " ---------------------------- " + password);
	 * System.out.println("in java controller"); Properties props = new
	 * Properties(); props.put("mail.smtp.host", "mail.Yash.com");
	 * props.put("mail.smtp.socketFactory.port", "587");
	 * props.put("mail.smtp.socketFactory.class",
	 * "javax.net.ssl.SSLSocketFactory"); props.put("mail.smtp.auth", "true");
	 * props.put("mail.smtp.port", "587");
	 * 
	 * Session sessionForProperties = Session.getDefaultInstance(props, new
	 ...
	 * velocityContext.put("memberEmailId", ((Member)
	 * session.getAttribute("loggedInUser")).getEmail());
	 ...
	 * velocityEngine.setProperty("resource.loader", "class"); velocityEngine
	 * .setProperty("class.resource.loader.class",
	 * "org.apache.velocity.runtime.resource.loader.ClasspathResourceLoader");
	 * 
	 * velocityEngine.init(); Template template =
	 * velocityEngine.getTemplate("emailtemplate.vm"); StringWriter stringWriter
	 * = new StringWriter(); // System.out.println(mail);
	 * template.merge(velocityContext, stringWriter);
	 * System.out.println(mail.getReceiver()); Message message = new
	 * MimeMessage(sessionForProperties); message.setFrom(new
	 * InternetAddress(email)); message.setRecipients(Message.RecipientType.TO,
	...
	public void getMessageDataFromMailFormAndSendMessage(
			@RequestParam("receiver") int trainerId,
			@RequestParam("subject") String reason,
			@RequestParam("body") List<Integer> documentsId,
			HttpServletResponse response, HttpSession session)
			throws IOException {
		System.out.println(trainerId + "  " + reason + "  " + documentsId);
		documentService.saveRequestForDocument(
				((Member) session.getAttribute("loggedInUser")).getId(),
				trainerId, documentsId, reason);
